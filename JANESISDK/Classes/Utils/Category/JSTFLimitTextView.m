...
@interface JSTFLimitTextView()


@end
@implementation JSTFLimitTextView

-(instancetype)initWithDelegate:(id<UITextViewExtensionDelegate>)delegate{
    if ([self init]) {
        self.responser = delegate;
        self.delegate = self;
        self.limitNum = 0;
    }
    return self;
 ...
 replacementText:(NSString *)text
{
    ...
    else
    {
        ...
        if (rg.length > 0)
        {
            ...
            if ([self.responser respondsToSelector:@selector(textDidChange:textLength:)]) {
                [self.responser textDidChange:self textLength:self.limitNum];
            }
        }
        return NO;
    }
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    //不让显示负数 口口日
    if ([self.responser respondsToSelector:@selector(textDidChange:textLength:)]) {
        [self.responser textDidChange:self textLength:MIN(self.limitNum,existTextNum)];
    }
}
